	                     ...
	                     int nCmdShow)
	{
	    ...
	    wincl.hbrBackground = CreateSolidBrush(RGB(0,0,255)); // SETTING BACKGROUND COLOR
	...
	LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
	    ...
	    static HWND textArea1, buttonSubmit, buttonClear,
	           textArea2, buttonRed, buttonGreen, buttonBlue, static1, static2;
	    ...
	    switch (message)                  /* handle the messages */
	    {
	        case WM_CREATE:
	        {
	            ...
	            textArea2 = CreateWindowEx(WS_EX_CLIENTEDGE, TEXT("EDIT"), TEXT(" "),
	                                       ...
	                                       0, 0, 0, 0,
	                                       hwnd, (HMENU)IDC_TEXT1, GetModuleHandle(NULL), NULL);
                static1 = CreateWindow (TEXT ("STATIC"), TEXT ("<< Header >>"),
                                    WS_CHILD | WS_VISIBLE | SS_CENTER,
                                    0, 0, 0, 0,
                                    hwnd, (HMENU) IDC_STATIC1, GetModuleHandle (NULL), NULL);

                static2 = CreateWindow (TEXT ("STATIC"), TEXT ("Some text !!!"),
                                    WS_CHILD | WS_VISIBLE | SS_CENTER,
                                    0, 0, 0, 0,
                                    hwnd, (HMENU) IDC_STATIC2, GetModuleHandle (NULL), NULL);

	            HGDIOBJ defaultFont = GetStockObject(DEFAULT_GUI_FONT);
	            SendMessage(textArea1, WM_SETTEXT, NULL, (LPARAM)"you can erase and write what you want");
	        ...
	        case WM_PAINT:
	        {
	            ...
	            SetTextColor(hdc, RGB(10, 0, 255));    /* the text from the center  */
	            DrawText(hdc, TEXT("Header"), -1, &rect, DT_SINGLELINE| DT_CENTER| DT_TOP);
	            EndPaint(hwnd, &ps);
	            break;
	        }
	        ...
	        case WM_SYSCOMMAND:
	        {
	            switch(wParam)
	            {
	                ...
	                case SC_MAXIMIZE:
	                {
	                 ...
	                 case SC_CLOSE:
	                 {
	                    if(MessageBox(hwnd, "sdasdasd?", "ASDF", MB_YESNO) == IDYES)
	                        {
	                            exit(1);
	                        }
	                        break;
	        ...
	        case WM_CTLCOLOREDIT:
	        {
	            ...
	            {
	                textArea1Brush = CreateSolidBrush(RGB(204, 153, 153));
	        ...
	        case WM_CTLCOLORSTATIC:
	        {
	            ...
	            {
	                HBRUSH hbr = (HBRUSH) DefWindowProc(hwnd, message, wParam, lParam);
	                SetTextColor((HDC) wParam, RGB(0, 163, 82));
	                return (BOOL) hbr;
	            }
	            break;
	        }
